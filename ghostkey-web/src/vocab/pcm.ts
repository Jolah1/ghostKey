...
export const pcm: Vocab = {
  ...
  drill: {
    ...
    walkthroughStep2Body: "Bitcoin itself dey enforce waiting period, and this page go show you the date. You just come back when e talk say make you. If dem record video message for you, e go play for here for the real day.",
    walkthroughStep3Title: "3. The money go come straight to you",
    walkthroughStep3Body: "You go paste address from any Bitcoin wallet for your phone, and e go arrive there. GhostKey never hold the money, so no company fit lose am or keep am from you.",
  ...
  claimCommon: {
    ...
    walletDesc: (network, examples) =>
      `Bitcoin wallet na app where you fit receive Bitcoin. You only need one wey fit receive on the ${network}. ${examples} all dey work.`,
    ...
    addressInstructions: (prefix) =>
      `Open any Bitcoin wallet and tap Receive to get address. Copy the long address wey start with ${prefix} and paste am below.`,
  ...
  guardian: {
    ...
    greeting: (heir, isHeir) => isHeir ? `Hello ${heir}.` : `De help ${heir}.`,
    heirBody: (heir) =>
  ...
  derivedClaim: {
    ...
    notYourEmail: "If that one no be your email, stop and reach out to the person wey send you this link.",
  ...
  heirRecoveryFile: {
    ...
    codeDescription: "The file dey locked with this code. We no go show am again, and we never store am. Keep the file and the code together.",
